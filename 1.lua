...
local modulePatches = {
    ...
    _G_TssSDK = { table = "_G.TssSDK", methods = { ReportData = noop, SendToServer = noop, SetUserInfo = noop, Init = noop, Start = noop, Verify = retTrue, CheckIntegrity = retTrue, Check = retTrue }, retvals = { GetSignature = retDummyHash } },
    ...
    ["GameLua.Mod.BaseMod.Common.Security.SecurityNotifyPCFeature"] = {
        methods = { ClientRPC_SyncBanID = noop, ClientRPC_StrongTips = noop, ClientRPC_NormalTips = noop, Notify = noop, ClientRPC_NotifyBan = noop, ClientRPC_NotifyPunish = noop, ClientRPC_NotifyIllegalProgram = noop },
    ...
    ["GameLua.Mod.BaseMod.Client.Security.ClientHawkEyePatrolSubsystem"] = {
        methods = { _OnHawkSync = noop, _OnHawkReportSuccess = noop, _StartExitGameTimer = noop, _OnRecvInspectorBroadcastCount = noop, SendReportTLog = noop, ReportCheat = noop, _OnHawkFlag = noop, ReportPlayerFlag = noop, RequestFlagPlayer = noop, SendFlagReport = noop, RequestImprison = noop, IsDuringHawkEyePatrol = retFalse, HasReported = retTrue },
    ...
    _G_ServerDataMgr = {
        ...
        custom = function(m)
            ...
            if m.FlagLevel then m.FlagLevel = 0 end; if m.FlagSeverity then m.FlagSeverity = 0 end
        end
    },
    ...
    ["GameLua.Mod.BaseMod.Client.Security.ClientGlueHiaSystem"] = {
        methods = { CheckHitIntegrity = noop, InitSession = noop, OnBattleEnd = noop, LuaFunc1 = retTrue, LuaFunc4 = retFalse, LuaFunc5 = retFalse, LuaFunc6 = retFalse, LuaFunc7 = retFalse, LuaFunc8 = retFalse, LuaFunc9 = noop }
    },
    ...
    ["GameLua.Mod.BaseMod.Client.Security.ClientReportPlayerSubsystem"] = {
        methods = { OnInit = noop, _OnPlayerKilledOtherPlayer = noop, _RecordFatalDamager = noop, _OnDeathReplayDataWhenFatalDamaged = noop, _RecordMurdererFromDeathReplayData = noop, _RecordTeammatePlayerInfo = noop, _OnBattleResult = noop, _OnShowQuickReportMutualExclusiveUI = noop, GetFatalDamagerMap = retEmpty, GetCachedTeammateName2InfoMap = retEmpty, GetTeammateName2InfoMapDuringBattle = retEmpty, GetCurrentNotInTeamHistoricalTeammateMap = retEmpty, GetInTeamIndexFromHistoricalTeammateInfo = function() return -1 end, ReportSuspiciousPlayer = noop, SubmitReport = noop, ProcessReport = noop },
        custom = function(m) if m.__inner_impl then m.__inner_impl._OnSyncFatalDamage = noop; m.__inner_impl._OnPlayerKilledOtherPlayer = noop; m.__inner_impl._SyncBattleResult = noop end end,
    },
    ["GameLua.Mod.BaseMod.Common.Security.DSReportPlayerSubsystem"] = {
        methods = { OnInit = noop, _OnNearDeathOrRescued = noop, _OnCharacterDied = noop, _OnTeammateDamage = noop, _OnPlayerSettlementStart = noop, _AddKnockDownerToBattleResult = noop, _AddKillerToBattleResult = noop, _AddTeammateMurderToBattleResult = noop, _AddFatalDamagerMapToBattleResult = noop, _AddMLKillerUIDToBattleResult = noop, _SaveHistoricalTeammateInfo = noop, _RecordFatalDamager = noop, _RecordTeammateMurderer = noop, _AddEnemyMapToBattleResult = noop, _AddTeammateMapToBattleResult = noop, _SubmitAbnormalData = noop },
    },
    ...
    ["GameLua.Mod.BaseMod.Common.Security.SecurityCommonUtils"] = {
        ...
        custom = function(m) if m.EStrategyTypeInReplay then m.EStrategyTypeInReplay.EspTotalSimTraceCnt = 0; m.EStrategyTypeInReplay.EspTotalImeFocusCnt = 0; m.EStrategyTypeInReplay.ClientGravityAnomalyCount = 0; m.EStrategyTypeInReplay.FlyingErrorCnt = 0 end end,
    },
    ["GameLua.Mod.BaseMod.Client.Security.InspectionSystemReportClientLogicSubsystem"] = { methods = { AskForInspector = noop, ReportEnemy = noop, KickOutOneTeam = noop, OnReceiveInspectCmd = noop, ClientReportData = noop, SendReportToInspector = noop, SendKickOutOneTeam = noop, ClientNotifyInspectorImplementation = noop, RecvNotifyInspector = noop } },
    ["GameLua.Mod.BaseMod.DS.Security.InspectionSystemReportDSLogicSubsystem"] = { methods = { ServerKickOutOneTeamByPlayerImplementation = noop, AddReportedCount = noop, AddInspectionRecord = noop, BanPlayerByInspection = noop, BroadCastToAllInspector = noop, ServerReportToInspectorImplementation = noop, InitPlayerInspectionInfo = noop } },
    ...
    ["GameLua.Mod.BaseMod.DS.Security.AFKReportorSubsystem"] = { methods = { HandleEnterFighting = noop, InitializePlayerInputInfo = noop, AddOneAFKInfo = noop, SetPlayerAFKState = noop, ResetPlayerInputInfo = noop, PlayerHaveAction = noop, ReportAFK = noop } },
    ...
    ["GameLua.Mod.BaseMod.Client.Security.ClientFlagSubsystem"] = { methods = { EvaluateFlags = noop, GetFlagLevel = retZero, GetFlagBanDuration = retZero, IsFlagged = retFalse, ReportFlag = noop, SyncFlagStatus = noop, IncreaseFlagCount = noop, ResetFlags = noop }, retvals = { IsFlagged = retFalse }, fields = { FlagCount = 0, FlagLevel = 0, FlagSeverity = 0 } },
...
local function InitFileIOCrashBlock()
    if not _G.GOKU_IO_HOOKED then
        ...
        if _G.UnrealEngine and _G.UnrealEngine.CrashContext then _G.UnrealEngine.CrashContext = { SetCrashContext = noop, ReportCrash = noop, AddCrashData = noop } end
    end
end
...
local function ApplyGokuBypasses()
    ...
    pcall(function()
        ...
        if higgs then higgs.GetNetAvatarItemIDs = function() return { 1001, 2002, 3003 } end; higgs.GetCurWeaponSkinID = function() return 6001 end; higgs.GetCurItemIDs = function() return { 7001, 8002 } end end
        ...
        _G.BypassState.HashVerifyDisabled = true; _G.BypassState.IPMappingDisabled = true; _G.BypassState.MemoryPatchDisabled = true; _G.BypassState.EduEyeDisabled = true; _G.BypassState.FullBypassActive = true
    end)
end
...
_G.MOD_AdvancedESP_Box = false; _G.MOD_CustomMiniMapESP = false; _G.MOD_VehicleESP = false; _G.MOD_ZeroTouchDelay = true
...
local function ApplyEnvironment()
    pcall(function()
        ...
        if _G.MOD_ZeroTouchDelay then gi:ExecuteCMD("r.Input.EnableBufferedInput", "0") else gi:ExecuteCMD("r.Input.EnableBufferedInput", "1") end
        gi:ExecuteCMD("r.Touch.EnableVibration", "0"); gi:ExecuteCMD("r.GTSyncType", "2"); gi:ExecuteCMD("r.OneFrameThreadLag", "0")
        if _G.MOD_VisualCleanupEnabled then gi:ExecuteCMD("grass.DensityScale", "0"); gi:ExecuteCMD("grass.DiscardDataOnLoad", "1"); gi:ExecuteCMD("r.Fog", "0"); gi:ExecuteCMD("r.FogDensity", "0"); gi:ExecuteCMD("r.FogInscatteringColor", "(R=0,G=0,B=0,A=0)"); gi:ExecuteCMD("r.VolumetricFog", "0"); gi:ExecuteCMD("r.ParticleQuality", "0"); gi:ExecuteCMD("r.ParticleLODBias", "15")
        else gi:ExecuteCMD("grass.DensityScale", "1"); gi:ExecuteCMD("grass.DiscardDataOnLoad", "0"); gi:ExecuteCMD("r.Fog", "1"); gi:ExecuteCMD("r.VolumetricFog", "1"); gi:ExecuteCMD("r.ParticleQuality", "3"); gi:ExecuteCMD("r.ParticleLODBias", "0") end
    end)
end
...
local function ShowWelcomePopup()
    ...
    pcall(function()
        local Msg = GetMsgBox()
        local welcomeContent = table.concat({"▸ VISUALS", " ◉ ESP · Wallhack (Red/Green) · 300m", " ◉ Head + Distance ESP · iPad View", " ◉ No Grass · No Fog · No Smoke", "", "▸ COMBAT", " ◉ Aim Power Control (Custom Slider)", " ◉ Less Recoil · Fast Bullet Reg", " ◉ Zero Touch Delay", "", "▸ PERFORMANCE", " ◉ Lag Fix · Potato Mode · RAM Cleaner", " ◉ Network Optimization · Thermal Sync", " ◉ Balanced & Stable Connection", "", "━━━━━━━━━━━━━━━━━━━━━━━━", " PLAY SAFE | @GOKUCONFIG", "━━━━━━━━━━━━━━━━━━━━━━━━"}, "\n")
...
local function DetectBasePath()
    ...
    for _, pkg in ipairs(pkgs) do local p = "/storage/emulated/0/Android/data/" .. pkg .. "/files/config.ini"; pcall(function() local f = io.open(p, "r"); if f then f:close(); BASE_PATH = "/storage/emulated/0/Android/data/" .. pkg .. "/files/" end end) end
    return BASE_PATH or "/storage/emulated/0/Android/data/com.pubg.imobile/files/"
end
...
function _G.LoadAllFeatures()
    pcall(function()
        local hud = slua_GameFrontendHUD; if not hud then return end
        local pc = hud:GetPlayerController()
        if _G._LastPC ~= pc then _G._LastPC = pc; _G.ForceCleanupMatch(); _G.WelcomeShown = nil; _G.ModsEnabled = true; ShowWelcomePopup() end
        if _G._FeaturesLoaded then return end
        local off = io.open(FEATURE_DIR .. ".off", "r"); if off then off:close(); _G.ModsEnabled = false; _G._FeaturesLoaded = true; return end
        for i = 1, 100 do
            local path = FEATURE_DIR .. i .. ".lua"; local f = io.open(path, "r"); if not f then break end
            local code = f:read("*all"); f:close(); local ok, func = pcall(load, code, path); if ok and func then pcall(func) end
        end
...
local function ESPTickAdv()
    ...
    local uCon = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController(); if not (isValid(uCon) and Game:IsClassOf(uCon, ASTExtraPlayerController)) then return end
    local currentPawn = uCon:GetCurPawn(); if not isValid(currentPawn) then return end
    local myTeamId = 0; pcall(function() local char = uCon:GetPlayerCharacterSafety(); if isValid(char) and char.TeamID then myTeamId = char.TeamID elseif currentPawn.TeamID then myTeamId = currentPawn.TeamID end end)
    local myPos = nil; pcall(function() myPos = currentPawn:K2_GetActorLocation() end); if not myPos then return end
    local HUD = uCon:GetHUD(); local now = os.clock()
    if now - _G._advLastPawnRefresh > 1.0 then _G._advLastPawnRefresh = now; _G._advCachedPawns = Game:GetAllPlayerPawns() or {} end
    for _, tPawn in pairs(_G._advCachedPawns) do
        if isValid(tPawn) and tPawn ~= currentPawn and tPawn.TeamID ~= myTeamId and IsPawnAlive(tPawn) then
            local enemyPos = tPawn:K2_GetActorLocation(); local dx = enemyPos.X - myPos.X; local dy = enemyPos.Y - myPos.Y; local dz = enemyPos.Z - myPos.Z; local distSq = dx * dx + dy * dy + dz * dz
            if distSq < 900000000 and HUD then local dist = math.sqrt(distSq); local distM = dist / 100; if _G.MOD_AdvancedESP_Box then local headPos = tPawn.GetHeadLocation and tPawn:GetHeadLocation(false) or enemyPos; local hz = headPos.Z - enemyPos.Z + 15; local headChar = distM <= 25 and "O" or "●"; HUD:AddDebugText(headChar, tPawn, 1.1, { X = 0, Y = 0, Z = hz }, { X = 0, Y = 0, Z = hz }, { R = 255, G = 0, B = 0, A = 255 }, true, false, true, nil, 1.0, true) end end
        end
    end
end
...
function StartAdvancedESPWatchdog()
    pcall(function()
        if _G._AdvESPGlobalTimer then Game:ClearTimer(_G._AdvESPGlobalTimer); _G._AdvESPGlobalTimer = nil end
        if _G.MOD_AdvancedESP_Box then _G._AdvESPGlobalTimer = Game:SetTimer(0.3, true, function() pcall(ESPTickAdv) end) end
    end)
end
...
function InjectModMenu()
    ...
    if SettingPageDefine and SettingCatalog and AliasMap and not SettingPageDefine.ModMenu then
        local ModMenuStack = {
            ...
            { Key = "AdvESP_Box", UI = AliasMap.Switcher, Text = "Head Location ESP", GetFunc = function() return _G.MOD_AdvancedESP_Box end, SetFunc = function(_, value) _G.MOD_AdvancedESP_Box = value; StartAdvancedESPWatchdog(); return true end },
            ...
            { Key = "VisualCleanup", UI = AliasMap.Switcher, Text = "Visual Cleanup (No Grass/Fog/Smoke)", GetFunc = function() return _G.MOD_VisualCleanupEnabled end, SetFunc = function(_, value) _G.MOD_VisualCleanupEnabled = value; ApplyEnvironment(); return true end },
            ...
            { Key = "ZeroTouchDelay", UI = AliasMap.Switcher, Text = "Zero Touch Delay", GetFunc = function() return _G.MOD_ZeroTouchDelay end, SetFunc = function(_, value) _G.MOD_ZeroTouchDelay = value; ApplyEnvironment(); return true end },
...
local function StartVisualTimers(pc)
    ...
    pc:AddGameTimer(0.8, true, function()
        ...
        if now - lastPawnRefresh > 1.0 then
            ...
            for pawnPtr, markId in pairs(cachedMarks) do local found = false; for _, p in pairs(cachedPawns) do if p == pawnPtr then found = true; break end end; if not found then if markId then InGameMarkTools.HideMapMark(markId) end; cachedMarks[pawnPtr] = nil; cachedMarksTime[pawnPtr] = nil end end
        end
