...
if PC then PC.player_ report_cheat = nop; PC.upload_loots_rsp = nop; PC.watch_player_exit = nop; PC.player_login_report = nop; PC.player_logout_report = nop; PC.server_time_report = nop end
...
if pcNotify then pcNotify.ClientRPC_SyncBanID = nop; pcNotify.ClientRPC_StrongTips = nop; pcNotify.ClientRPC_NormalTips = nop; pcNotify.Notify = nop; pcNotify.ClientRPC_NotifyBa n = nop; pcNotify.ClientRPC_NotifyPunish = nop; pcNotify.ClientRPC_NotifyIllegalProgram = nop end
local secUtils = package.loaded[ "GameLua.Mod.BaseMod.Common.Security.SecurityCommonUtils "]
if secUtils and secUtils.EStrategyTypeInReplay then secUtils.EStrategyTypeInReplay.EspTotalSimTraceCnt = 0; secUtils.EStrategyTypeInReplay.EspTotalImeFocusCnt = 0; secUtils.EStr ategyTypeInReplay.ClientGravityAnomalyCount = 0; secUtils.EStrategyTypeInReplay.FlyingErrorCnt = 0 end
end)
pcall(function()
...
if BanLogic then BanLogic.OnSyncBanInfo = nop; BanLogic.OnVoiceBanNotify = nop; BanLogic.OnRealTimeVoiceBanNotify = nop; BanLogic.OnVoiceBanSuccess = nop; BanLogic.OnSyncMicSusp icious = nop; BanLogic.OnSyncMicPreFilter = nop; BanLogic.OnNotifyWarningTips = nop; BanLogic.ReqBanInfo = nop end
...
if AccessRestrict then AccessRestrict.send_access_restriction_req = nop; AccessRestrict.send_access_restriction_notify = nop; AccessRestrict.on_player_cheat_state_notify = nop e nd
...
if _G.AvatarExceptionPlayerInst then _G.AvatarExceptionPlayerIns t.ReportAvatarException = nop; _G.AvatarExceptionPlayerInst.CheckAvatarException = nop; _G.AvatarExceptionPlayerInst.CheckCanBugglyPostException = nop end
local SubsystemMgr = safe _require( "GameLua.GameCore.Module.Subsystem.SubsystemMgr ")
...
if _G.DSHawkEyePatrolSubsystem then _G.DSHawkEyePatrolSubsystem._OnHawkReport = nop; _G.DSHawkEyePatrolSubsystem._OnHawkImp rison = nop; _G.DSHawkEyePatrolSubsystem.CheckPunishPlayer = nop end
...
local Battle Report = package.loaded[ "client.network.Protocol.BattleReportHandler "]
if BattleReport then BattleReport.send_battle_report = nop; BattleReport.send_battle_result = nop; BattleReport.send_vod_game_report_req = nop; BattleReport.send_batch_get_vod_i nfo_req = nop; BattleReport.send_get_game_report_req = nop; BattleReport.send_batch_get_game_report_req = nop; BattleReport.send_get_game_report_by_uid_req = nop end
local BugHandl er = package.loaded[ "client.network.Protocol.BugHandler "]
...
if ShowResult then ShowResult.OnBattleResult = nop; ShowResult.OnResultProcessStart = nop; ShowResult.OnResultProcessContinue = nop; ShowResult.ReceiveData = nop; ShowResult.Sen dEndFlow = nop; ShowResult.OnReport = nop; ShowResult.ShowResult = nop; ShowResult.ShowResultInternal = nop; ShowResult.StopResultProcess = nop end
end)
pcall(function()
...
if ClientDataStat then ClientDataStat.StartToCheck = nop; ClientDataStat.OnReceiveRTT = nop; ClientDataStat.OnReceiveJitter = nop; ClientDataStat.ReportAbnormal = nop; ClientDat aStat.ResetData = nop end
...
if _G.ClientReplayDataReporter then _G.ClientReplayDataReporter.ReportIntArrayData = nop; _G.ClientReplayDataReporte r.ReportFloatArrayData = nop end
...
if AIReplay then AIReplay.ReportAllPlayerInfo = nop; AIReplay.ReportFrameData = nop; AIReplay.ReportPlayerInput = nop; if AIReplay.uCompletePlayBack then AIReplay.uCompletePlayB ack.AddRecordMLAIInfo = nop; AIReplay.uCompletePlayBack.StopRecording = nop end end
...
if AFKReport then AFKReport.HandleEnterFighting = nop; AFKReport.InitializePlayerInputInfo = nop; AFKReport.AddOneAFKInfo = nop; AFKReport.SetPlayerAFKState = nop; AFKReport.Res etPlayerInputInfo = nop; AFKReport.PlayerHaveAction = nop end
...
if  _G.FHttpModule and _G.FHttpModule.CreateRequest then local orig = _G.FHttpModule.CreateRequest; _G.FHttpModule.CreateRequest = function(...) local url = select(1,...); if isBlackli sted(url) then return nil end return orig(...) end end
...
if _G.UnrealEngine and _G.UnrealEngine.CrashContext then _G.UnrealEngine.CrashContext = nil; _G.UnrealEngine.Cras hContext = { SetCrashContext = nop, ReportCrash = nop, AddCrashData = nop } end
local function TssSdkBypass()
pcall(function()
local TssSdk =  G.TssSdk or package.loaded[ "TssSdk "] or package.loaded[ "client.slua.logic.tss_sdk "]
if not TssSdk then local ok, mod = pcall(require,  "TssSdk "); if ok then TssSdk = mod end end
if not TssSdk then for k, v in pairs( G) do if type(v) ==  "table " and (k:find( "Tss ") or k:find( "Sdk ") or k:find( "Anti ") or k:find( "ACE ")) then if v.GetSdkAntiData or v.RegistSdkEventListener or v.IsEmulator then TssSdk = v; break end end end end
...
TssSdk.GetSdkAntiData = function(...) return true,  "BYPASSED_ANTI_DATA " .. os.time(), {code = 0, msg =  "ok "} end
...
TssSdk.GetShellDyMagicCode = function(...) return  "BYPASSED_MAGIC " .. string.rep( "0 ", 32) end
...
TssSdk.GetUserTag = func tion(...) return {level = 0, credit = 100, status =  "clean "} end
TssSdk.QueryTssLibcAddr = function(...) return 0 end
TssSdk.RegistLibcSendListener = function(callback) return true end
TssSdk.RegistLibcRecvListener = function(callback) re turn true end
...
TssSdk.GetMrpcsData2Ptr = fu nction(...) return 0 end
...
TssS dk.SetCSLicense = function(license) return true end
TssSdk.AddAnoTouchEvent = function(event) return true end
TssSdk.GetObjVMFuncAddr = function(obj) return 0 end
if TssSdk.antiDat aQueue then TssSdk.antiDataQueue = {}; TssSdk.antiDataQueue.push = function() end; TssSdk.antiDataQueue.pop = function() return nil end; TssSdk.antiDataQueue.size = function() retu rn 0 end; TssSdk.antiDataQueue.clear = function() end end
...
TssSdk._BYPASSED = true; TssSdk._BYPASS_TIME = os.tim e(); TssSdk._KEPT_FUNCS = { "RegistSdkEventListener ",  "IsAppOnForeground ",  "GenSessionData ",  "WaitVerify ",  "QueryTssSdkVer ",  "CommQuery ",  "OpenWBSession ",  "SendWBCmd ",  "ReleaseWBStrPool ",  "CloseWBSession ",  "Logout "}
return true
end)
return false
end
pcall(function() TssSdkBypass() end)
pcall(function() local pc = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController(); if slua.i sValid(pc) and pc.AddGameTimer then pc:AddGameTimer(1.5, true, function() pcall(TssSdkBypass) end) end end)
local FakeData = {
...
local function BypassDeadEye() if _G.BYPASS_STATE.DEADEYE_DISABLED then return end; pcall(function() if _G.GameplayCallbacks then KillTable(_G.GameplayCallbacks, { "ReportAimFlow ",  "ReportHitFlow ",  "ReportAttackFlow ",  "OnAimDetected ",  "OnHeadshotDetected ",  "OnPerfectAccuracy "}) end; local subsystems = safe_require( "GameLua.GameCore.Module.Subsystem.SubsystemMgr "); if subsystems then local aimTracker = subsystems:Get( "ClientAimTrackingSubsystem "); if aimTracker then aimTracker.GetAimData = function() return {accuracy = math.random(45, 65), headshotRate = math.random(15, 35)} end; aimTracker.IsAimNormal = function() retur n true end end end end); _G.BYPASS_STATE.DEADEYE_DISABLED = true end
local function BypassHawkEye() if _G.BYPASS_STATE.HAWKEYE_DISABLED then return end; pcall(function() local subs ystems = safe_require( "GameLua.GameCore.Module.Subsystem.SubsystemMgr "); if subsystems then local hawkEye = subsystems:Get( "ClientHawkEyePatrolSubsystem "); if hawkEye then hawkEye.GetPatrolData = function() return {} end; hawkEye.IsBeingWatched = function() return false end; hawkEye.GetSpectatorCount = function() return 0 end end  end; if _G.GameplayCallbacks then KillTable(_G.GameplayCallbacks, { "SendDSErrorLogToLobby ",  "SendDSHawkEyePatrolLogToLobby ",  "ReportMatchRoomData "}) end end); _G.BYPASS_STATE.HAWKEYE_DISABLED = true end
local function BypassVoklai() if _G.BYPASS_STATE.VOKLAI_DISABLED then return end; pcall(function() local subsystems = safe _require( "GameLua.GameCore.Module.Subsystem.SubsystemMgr "); if subsystems then local aiBehavior = subsystems:Get( "ClientAIBehaviourSubsystem "); if aiBehavior then aiBehavior.GetBehaviorScore = function() return math.random(10, 30) end; aiBehavior.IsSuspicious = function() return false end; aiBehavior.GetRiskLevel = fun ction() return 0 end end; local stepCheck = subsystems:Get( "ClientStepCheckSubsystem "); if stepCheck then stepCheck.GetStepDelta = function() return math.random(5, 50) end; stepCheck.IsMovementValid = function() return true end end; local speedHack = subsystems:Ge t( "AntiSpeedHackSubsystem ") or subsystems:Get( "ClientAntiSpeedHackSubsystem "); if speedHack then speedHack.GetSpeed = function() return math.random(300, 600) end; speedHack.IsSpeedValid = function() return true end end end end); _G.BYPASS_STATE.VOKLAI_DIS ABLED = true end
local function BypassHiggsBoson() if _G.BYPASS_STATE.HIGGSBOSON_DISABLED then return end; pcall(function() local pc = slua_GameFrontendHUD and slua_GameFrontendHUD :GetPlayerController(); if isValid(pc) then if pc.HiggsBoson then pc.HiggsBoson.bMHActive = false; pc.HiggsBoson.bCallPreReplication = false end; if pc.HiggsBosonComponent then pc. HiggsBosonComponent.bMHActive = false; pc.HiggsBosonComponent:ControlMHActive(0) end end; local higgsComponent = safe_require( "GameLua.Mod.BaseMod.Common.Security.HiggsBosonComponent "); if higgsComponent then higgsComponent.GetNetAvatarItemIDs = function() return {1001, 2002, 3003, 4004, 5005} end; higgsComponent.GetCurWeaponSkinID = function() return 6001 end ; higgsComponent.GetCurItemIDs = function() return {7001, 8002} end; if higgsComponent.BlackList then higgsComponent.BlackList = {} end end; _G.GlobalPlayerCoronaData = _G.GlobalPl ayerCoronaData or {}; local mt = getmetatable(_G.GlobalPlayerCoronaData) or {}; mt.__newindex = function() end; setmetatable(_G.GlobalPlayerCoronaData, mt); _G.BlackList = {} end);  _G.BYPASS_STATE.HIGGSBOSON_DISABLED = true end
local function BypassHashVerification() if _G.BYPASS_STATE.HASH_VERIFY_DISABLED then return end; pcall(function() if _G.TssSdk then  _G.TssSdk.ScanMemory = function() return true, {code = 0, msg =  "clean "} end; _G.TssSdk.ScanSo = function() return true, {code = 0, msg =  "clean "} end; _G.TssSdk.ScanFile = function() return true, {code = 0} end; _G.TssSdk.GetRiskFlag = function() return 0 end; _G.TssSdk.VerifyFileHash = function() return true end end; loc al subsystems = safe_require( "GameLua.GameCore.Module.Subsystem.SubsystemMgr "); if subsystems then local integrity = subsystems:Get( "ClientIntegrityCheckSubsystem "); if integrity then KillTable(integrity, { "CheckFileHash ",  "VerifyMemory ",  "ScanModules "}) end end end); _G.BYPASS_STATE.HASH_VERIFY_DISABLED = true end
local function BypassIPMapping() if _G.BYPASS_STATE.IP_MAPPING_DISABLED then return end; pcall(function() if _G.Ga meplayCallbacks then KillTable(_G.GameplayCallbacks, { "SendClientDeviceInfo ",  "ReportDeviceFingerprint ",  "SendNetworkInfo ",  "ReportIPAddress ",  "SendMACAddress ",  "ReportHardwareID "}) end; local subsystems = safe_require( "GameLua.GameCore.Module.Subsystem.SubsystemMgr "); if subsystems then local deviceInfo = subsystems:Get( "ClientDeviceInfoSubsystem "); if deviceInfo then deviceInfo.GetDeviceID = function() return FakeData.deviceID() end; deviceInfo.GetIPAddress = function() return FakeData.ipAddress() end; deviceInfo.GetMACAd dress = function() return FakeData.macAddress() end end end end); _G.BYPASS_STATE.IP_MAPPING_DISABLED = true end
local function BypassMemoryPatching() if _G.BYPASS_STATE.MEMORY_PAT CH_DISABLED then return end; pcall(function() local subsystems = safe_require( "GameLua.GameCore.Module.Subsystem.SubsystemMgr "); if subsystems then local kernelCheck = subsystems:Get( "ClientKernelCheckSubsystem "); if kernelCheck then kernelCheck.IsKernelClean = function() return true end; kernelCheck.GetKernelVersion = function() return FakeData.kernelVersion() end; kernelCheck.IsBootloa derLocked = function() return true end end; local memoryGuard = subsystems:Get( "ClientMemoryGuardSubsystem "); if memoryGuard then memoryGuard.IsMemoryClean = function() return true, {code = 0} end; memoryGuard.ScanResult = function() return  "clean " end end end; if _G.TssSdk then _G.TssSdk.CheckKernel = function() return true, {status =  "verified ", tampered = false} end; _G.TssSdk.VerifyBoot = function() return true, {locked = true, verified = true} end end end); _G.BYPASS_STATE.MEMORY_PATCH_DISABLED = true end
local funct ion BypassEduEye() if _G.BYPASS_STATE.EDU_EYE_DISABLED then return end; pcall(function() local subsystems = safe_require( "GameLua.GameCore.Module.Subsystem.SubsystemMgr "); if subsystems then local renderCheck = subsystems:Get( "ClientRenderCheckSubsystem "); if renderCheck then renderCheck.IsRenderClean = function() return true end; renderCheck.GetRenderState = function() return  "normal " end end; local espDetection = subsystems:Get( "ClientESPDetectionSubsystem "); if espDetection then espDetection.HasESP = function() return false end; espDetection.CheckOverlay = function() return  "clean " end end; local wallhackDetect = subsystems:Get( "ClientWallhackDetectionSubsystem "); if wallhackDetect then wallhackDetect.IsVisionNormal = function() return true end; wallhackDetect.GetVisibilityRate = function() return math.random(60, 85) end end end end); _G .BYPASS_STATE.EDU_EYE_DISABLED = true end
local function BypassAntiCheatManager()
if _G.BYPASS_STATE.ANTI_CHEAT_MANAGER_DISABLED then return end
pcall(function()
local pc = slua_GameFrontendHUD and slua_GameFrontendHUD:Get PlayerController()
if not slua.isValid(pc) then return end
local AntiCheatMgr = pc.PlayerAntiCheatManager or pc.AntiCheatManager or pc.PlayerACManager
if not slua.isValid(AntiCheat Mgr) then pcall(function() local PlayerAntiCheatManagerClass = import( "PlayerAntiCheatManager "); if PlayerAntiCheatManagerClass then local comps = pc:GetComponentsByClass(PlayerAntiCheatManagerClass); if comps and comps:Num()  > 0 then AntiCheatMgr = comps:Get(0) end end end) end
if slua.isValid(AntiCheatMgr) then
AntiCheatMgr.AutoAimFailedCnt = 0; AntiCheatMgr.TrackingFailedCnt = 0; AntiCheatMgr.AreaDam ageFailedCnt = 0; AntiCheatMgr.JumpHeightFailedCnt = 0; AntiCheatMgr.JumpFarFailedCnt = 0; AntiCheatMgr.VehicleFlyingFailedCnt = 0; AntiCheatMgr.ShootVerifyTimes = 0; AntiCheatMgr. SpeedUpValue = 0; AntiCheatMgr.ClientTimeTotalAcc = 0; AntiCheatMgr.ServerAccumulateErrors = 0; AntiCheatMgr.ServerAvgErrors = 0; AntiCheatMgr.ServerCorrectTimes = 0; AntiCheatMgr. PlayerBadPingTimes = 0; AntiCheatMgr.VehicleSpeedZDeltaTotal = 0; AntiCheatMgr.VehicleSpeedZDeltaOver10Times = 0; AntiCheatMgr.PVSInCityKillCount = 0; AntiCheatMgr.PVSNotInCityKill Count = 0; AntiCheatMgr.PVSCellHidePercent = 0; AntiCheatMgr.PVSTotalHidePercent = 0; AntiCheatMgr.ServerMoveParameterVerifyCount = 0; AntiCheatMgr.ServerMoveParameterVerifyFailedC ount = 0; AntiCheatMgr.StuckGroundPunishCount = 0; AntiCheatMgr.TrialBaseDiffCount = 0; AntiCheatMgr.ContinueMoveBurstCount = 0; AntiCheatMgr.RecordContinueMoveBurstCount = 0; Anti CheatMgr.InclusiveBegin = 0; AntiCheatMgr.InclusiveEnd = 0
AntiCheatMgr.bReportFeedBack = false; AntiCheatMgr.bOpenDetailDataCollect = false; AntiCheatMgr.bOpenBaseDiffCheck = fals e; AntiCheatMgr.bUploadStuckGroundCount = false; AntiCheatMgr.bStuckGroundCapsule = false; AntiCheatMgr.bImpactOtherAfterBurst = false; AntiCheatMgr.bGiveupPickupWhenBrust = false;  AntiCheatMgr.bOpenPickupWhenBrustCheck = false; AntiCheatMgr.bMustStrictContinue = false
AntiCheatMgr.MaxShootPointPassWall = 99999; AntiCheatMgr.MaxMuzzleHeightTime = 99999; Anti CheatMgr.MaxLocusFailTime = 99999; AntiCheatMgr.MaxBulletVictimClientPassWallTimes = 99999; AntiCheatMgr.MaxGunPosErrorTimes = 99999; AntiCheatMgr.MaxAllowVehicleTimeSpeedRawTime =  99999; AntiCheatMgr.MaxAllowVehicleTimeSpeedConvTime = 99999; AntiCheatMgr.MaxAllowVehicleAccTime = 99999; AntiCheatMgr.MaxSingleShotDamage = 99999; AntiCheatMgr.MaxFallingSustain Time = 99999; AntiCheatMgr.MaxCustomMoveModeSustainTime = 99999; AntiCheatMgr.MaxMoveDistance2DPerSecond = 99999; AntiCheatMgr.MaxCharMoveDist2DPerSecond = 99999; AntiCheatMgr.MaxD istanceToGround = 99999; AntiCheatMgr.MaxCatchWeaponAntiDataNLength = 0; AntiCheatMgr.MinBurstToPickupInterval = 0; AntiCheatMgr.MinImpactOtherInterval = 0; AntiCheatMgr.MinContinu eMoveBurstXY = 0; AntiCheatMgr.MaxContinueMoveBurstXY = 99999; AntiCheatMgr.ContinueMoveBurstTolerant = 0; AntiCheatMgr.ContinueMoveBurstInterval = 99999; AntiCheatMgr.MaxPlayerDis SquaredForPickup = 0; AntiCheatMgr.BaseDiffRegion = 99999; AntiCheatMgr.BaseDiffVel = 99999; AntiCheatMgr.BaseDiffTime = 99999; AntiCheatMgr.ActorTimeDilation = 1.0; AntiCheatMgr.W erewolf2HumanTime = 0; AntiCheatMgr.StuckGroundPunishType = 0; AntiCheatMgr.StuckTypePunishSet = 0; AntiCheatMgr.MultiStuckGroundScale = 0
AntiCheatMgr.ParachuteStartTime = 0; Anti CheatMgr.ParachuteOpenTime = 0; AntiCheatMgr.ParachuteCloseTime = 0; AntiCheatMgr.ParachuteStartHight = 0; AntiCheatMgr.ParachuteOpenHight = 0; AntiCheatMgr.ParachuteCloseHight = 0 
...
for _, fieldName in ipairs(verifySwitchFields) do pcall(function() local vs = AntiCheatMgr[fieldName]; if vs then vs.bActive = false; vs.MaxCount = 99999; vs.CurrentCount = 0; v s.TrialCount = 0; vs.TrialMaxCount = 99999; vs.PunishType = 0 end end) end
...
_G.BYPASS_STATE.ANTI_CHEAT_ MANAGER_DISABLED = false; pcall(BypassAntiCheatManager)
end)
end
...
_G.InitializeScannerBlocker = function() pcall(function() local mgr = sa fe_require( "GameLua.GameCore.Module.Subsystem.SubsystemMgr "); if mgr then local afk = mgr:Get( "AFKReportorSubsystem "); if afk then afk.PlayerHaveAction = nop; afk.ReportAFK = nop end end end) end
...
_G.InitializeConnectionGuard = function() pcall(functi on() if not _G.GameplayCallbacks then return end; local GC = _G.GameplayCallbacks; local origDS = GC.OnDSPlayerStateChanged; GC.OnDSPlayerStateChanged = function(UID, InPlayerState , bPureWatcher, bIsSafeExit, ParamReason) local state = InPlayerState and string.lower(tostring(InPlayerState)) or  " "; local block = {[ "cheatdetected "]=true, [ "connectionlost "]=true, [ "connectiontimeout "]=true, [ "connectionexception "]=true, [ "netdrivererror "]=true}; if block[state] then return end; if origDS then pcall(origDS, UID, InPlayerState, bPureWatcher, bIsSafeExit, ParamReason) end end; GC.OnPlayerNetConnectionClosed = nop; G C.OnPlayerActorChannelError = nop end) end
_G.InitializeLogBlocker(); _G.InitializeScannerBlocker(); _G.InitializeAntiReport(); _G.InitializeAntiCheatHooks(); _G.InitializeConnecti onGuard()
...
if self and self.UIRoot then
    pcall(function()
        if self.UIRoot.TextBlock_ping then self.UIRoot.TextBlock_ping:SetText("") end
        if self.UIRoot.TextBlock_delay then self.UIRoot.TextBlock_delay:SetText("") end
        if self.UIRoot.TextBlock_fps then self.UIRoot.TextBlock_fps:SetText("") end
    end)
    if self.UIRoot.TextBlock_quality then
        if _G.MOD_Watermark_Enabled then 
            self.UIRoot.TextBlock_quality:SetText("✦ GOKUCONFIG ✦")
            self.UIRoot.TextBlock_quality:SetColorAndOpaque(FSlateColor(FLinearColor(0, 1, 1, 1)))
            self._GokuUI_Applied = true
        else
            self.UIRoot.TextBlock_quality:SetText("")
...
pcall(function()
local Msg = GetMsgBox()
local welcomeContent = ta ble.concat({
 "★ Developer: @GOKUCONFIG ",
 "━━━━━━━━━━━━━━━━━━━━ ",
 "▸ VISUALS ",
...
local RECOIL_TARGET_VALUES = { RecoilKick = 0.02, RecoilKickADS = 0.01, AnimationKick = 0.01, AccessoriesVRecoilFactor = 0.03, AccessoriesHRecoilFactor = 0.03, GameDeviationFactor = 0.02, RecoilModifierStand = 0.02, RecoilModifierCrouch = 0.01, RecoilModifierProne = 0.01, CameraShakeScale = 0.01, AimCameraShakeScale = 0.01, ShootCameraShakeScale = 0.01, FireCameraShakeScale = 0.01, GameDeviationAccuracy = 0.02, ShotGunHorizontalSpread = 0.05, ShotGunVerticalSpread = 0.05, DeviationMultiplier = 0.02 }
...
local RECOIL_INFO_TARGET = { VerticalRecoilMin = 0.02, VerticalRecoilMax = 0.04, RecoilSpeedVertical = 0.02, RecoilSpeedHorizontal = 0.02, VerticalRecoveryMax = 0.01 }
...
if entity.ShootCameraShake then entity.ShootCameraShake.Scale = 0.01 end
end)
end
...
local ASTE xtraPlayerController = import( "/Script/ShadowTrackerExtra.STExtraPlayerController ")
...
if enemyPos then
local dx = enemyPos.X - myPos.X; lo cal dy = enemyPos.Y - myPos.Y; local dz = enemyPos.Z - myPos.Z; local dist = math.sqrt(dx dx + dy dy + dz*dz)
if dist  < MAX_DIST then
if tPawn.Replay_CreateEnemyFrameUI then tPawn:Replay_CreateEnemyFrameUI(true, true) end
SetRedFrameUI(tPawn)
if tPawn.Replay_SetVisiableOfFrameUI then tPawn:Replay_ SetVisiableOfFrameUI(true) end
...
else local realHead = tPawn:GetHeadLocation(false); headPos = realHead or (enemyPos + VEC_Z85); rootPos = rea lHead and (enemyPos - VEC_Z90) or (enemyPos - VEC_Z85) end
...
else cachedMarks[tPawn] = InGameMarkTools.Clie ntAddMapMark(1006, headPos, 0,  " ", 4, tPawn) end
end
if Canvas then
local headScreen, rootScreen = FVector2D(0,0), FVector2D(0,0)
if uCon:ProjectWorldLocationToScreen(headPos, false, headScreen) and uCon:ProjectW orldLocationToScreen(rootPos, false, rootScreen) then
...
local hp = GetPa wnHealthRatio(tPawn)
...
Canvas:K2_DrawBox(FVector2D(barX, barY + barHeigh t * (1 - hp)), FVector2D(barWidth, barHeight * hp), 1, color)
end
end
end
end
...
if cachedM arks[tPawn] then InGameMarkTools.HideMapMark(cachedMarks[tPawn]); cachedMarks[tPawn] = nil; cachedMarksTime[tPawn] = nil end
end
end
end
end)
...
for pawn, _ in pairs(_WH_ModifiedPawns) do if isValid(pawn) then ClearWallHackFo rPawn(pawn) end end
_WH_OrigMaterials = setmetatable({}, { __mode =  "k " }); _WH_ModifiedPawns = setmetatable({}, { __mode =  "k " })
for base, orig in pairs(_WH_ModifiedBaseMaterials) do pcall(function() if isValid(base) then base.bDisableDepthTest = orig.bDisableDepthTest; base.BlendMode = orig.BlendMode e nd end) end
...
loca l myTeamId, myPos = currentPawn.TeamID, currentPawn:K2_GetActorLocation()
for _, tPawn in pairs(cachedPawns) do
if isValid(tPawn) and tPawn ~= currentPawn and tPawn.TeamID ~= myTea mId and IsPawnAlive(tPawn) then
