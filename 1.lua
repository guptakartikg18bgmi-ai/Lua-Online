-- ============================================================================
-- ✦ GOKU FRAMEWORK + PVT 18-LAYER BYPASS [NATIVE BRIDGE & SERVER SYNC FIXED] ✦
-- ============================================================================
if not _G.GOKU_ONE_TIME_INIT_DONE then
    ...
    pcall(function()
        ...
        if Msg and Msg.Show then
            ...
            "★ Bypass    : NATIVE BRIDGE SAFE\n" ..
            "★ Engine    : SERVER-SYNC ALIGNED\n\n" ..
...
pcall(function()
    ...
    if callbacks then
        local kills = {"SendDSErrorLogToLobby", "SendDSHawkEyePatrolLogToLobby", "SendSecTLog", "SendDataMiningTLog", "SendActivityTLog", "SendClientMemUsage", "SendClientFPS", "OnClientCrashReport", "OnNetworkLossDetected", "ReportMatchRoomData", "ReportPlayersPing", "SendClientStats", "SendServerAvgTickDelta", "OnPlayerActorChannelError", "OnPlayerRPCValidateFailed"}
        -- REMOVED SendTssSdkAntiDataToLobby and ReportHitFlow from kills to prevent server desync
...
pcall(function()
    ...
    if clientReport then local funcs = {"OnInit", "_OnPlayerKilledOtherPlayer", "_RecordFatalDamager", "ReportSuspiciousPlayer", "SubmitReport", "_OnBattleResult", "_RecordTeammatePlayerInfo", "_OnDeathReplayDataWhenFatalDamaged", "_RecordMurdererFromDeathReplayData"}; for _, fn in ipairs(funcs) do if clientReport[fn] then clientReport[fn] = nop end end end
...
local globalFuncs = {"ReportTLogEvent", "SendTlog", "SendClientStats", "ReportAvatarException", "SendComplaintReq", "SubmitReport", "ReportSuspiciousPlayer", "OnSyncBanInfo", "OnVoiceBanNotify", "SendSecTLog", "MarkSuspiciousPlayer", "ReportPlayerBehaviorData", "CheckCompliance", "ReportIllegalProgram", "UploadVoiceLog"}
-- REMOVED ReportHitFlow, SendPacket from globalFuncs to allow server bullet sync
...
-- [FIXED TSSSDK NATIVE BRIDGE]
-- DO NOT override core binary bridge functions with Lua strings. It causes Native Memory Access Violation (Security Violation).
local function TssSdkBypass()
    pcall(function()
        ...
        -- Only safe Lua-side overrides. No "BYPASSED" strings returned to C++.
        TssSdk.GameScreenshot = function(...) return nil end
        TssSdk.GameScreenshot2 = function(...) return nil end
...
local function BypassDeadEye() if _G.BYPASS_STATE.DEADEYE_DISABLED then return end; pcall(function() if _G.GameplayCallbacks then KillTable(_G.GameplayCallbacks, {"ReportAimFlow", "ReportAttackFlow", "OnAimDetected", "OnHeadshotDetected", "OnPerfectAccuracy"}) end; local subsystems = safe_require("GameLua.GameCore.Module.Subsystem.SubsystemMgr"); if subsystems then local aimTracker = subsystems:Get("ClientAimTrackingSubsystem"); if aimTracker then aimTracker.GetAimData = function() return {accuracy = math.random(45, 65), headshotRate = math.random(15, 35)} end; aimTracker.IsAimNormal = function() return true end end end end); _G.BYPASS_STATE.DEADEYE_DISABLED = true end
...
-- [FIXED ANTI-CHEAT MANAGER SYNC]
-- DO NOT set bActive = false. Server checks config hash. Only freeze CurrentCount to 0.
        ...
        if slua.isValid(AntiCheatMgr) then
            ...
            for _, fieldName in ipairs(verifySwitchFields) do 
                pcall(function() 
                    ...
                    if vs then 
                        -- ONLY FREEZE COUNTERS. DO NOT TOUCH bActive OR MAXCOUNT.
                        vs.CurrentCount = 0
                        vs.TrialCount = 0
                    end 
                end) 
...
pcall(function()
    ...
    -- [FIXED NETWORK PACKET BLOCKER FOR FAKE DAMAGE]
    -- Unblocked: ReportAttackFlow, ReportFireArms, ReportHitFlow, ReportVerifyInfoFlow, ReportMrpcsFlow, ReportAimFlow, ReportCircleFlow, ReportJumpFlow, ReportEquipmentFlow
    -- Server NEEDS these packets to register damage and validate bullet physics.
    pcall(function() 
...
local function ShowWelcomePopup()
    ...
    pcall(function()
        ...
        " ",
        "▸ COMBAT",
        " ✦ Custom Aim Assist • Server-Sync Recoil",
        " ",
        "▸ PERFORMANCE",
...
-- [FIXED SERVER-ALIGNED MICRO-RECOIL]
-- 0.15 (15%) is the sweet spot. Server accepts it as legit physics, but it feels like zero recoil to the player.
...
print("[MOD] ✅ GOKU + NATIVE BRIDGE & SERVER SYNC [FIXED BUILD] LOADED SUCCESSFULLY!")
