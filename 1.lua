-- ============================================================================
-- ✦ GOKU ULTRA SHIELD – FINAL SCRIPT ✦
-- Complete 7-layer anti-cheat bypass + all visual & combat mods
-- Covers every subsystem + full MD5/CRC fakers + CoronaData protection.
-- Gokuba/TSS bypass + GameReport + DSFightTLog + AFK + ClientDataStats
-- Fully optimized for minimal lag. Zero crash on init.
-- [MAP MARKER FIX: 200m range, no lag – MaxShowDistance unlimited + manual limit]
-- ============================================================================
...
local modulePatches = {
    ...
    ["GameLua.Mod.BaseMod.DS.Security.DSGlueHiaSystem"] = {
        methods = {
            OverrideTriggerRealtimeVerify = noop, PostInitSystems = noop,
            PostChangeLastFrameFightingState = noop,
        },
    },
    ["GameLua.Mod.BaseMod.DS.Security.DSPVSSubsystem"] = {
        methods = {
            RefreshGameModeModType = noop, EditorTriggerCoronaLabVerify = noop,
        },
    },
    ["GameLua.Mod.BaseMod.DS.Security.DSMaliciousTeammateDetectionSubsystem"] = {
        methods = {
            _OnNearDeathOrRescued = noop, _OnCharacterDied = noop,
            _HandleTeammateDirectDamage = noop, _RecordJustifiableDefense = noop,
        },
    },
    ["GameLua.Mod.BaseMod.DS.Security.DSCreditTLogSubsystem"] = {
        methods = {
            ReportCreditTLog = noop, SendCreditData = noop,
        },
    },
    ["GameLua.Mod.BaseMod.DS.Security.DSDynamicDispatched"] = {
        methods = {
            RefreshFrameTime = noop,
        },
    },
    ["GameLua.Mod.BaseMod.Client.Security.SoundMonitorSubsystem"] = {
        methods = {
            ReportSoundAnomaly = noop, DetectSoundHack = noop,
        },
    },
    ["GameLua.Mod.BaseMod.Client.Security.TransparentUIModeSubsystem"] = {
        methods = {
            CheckTransparentUI = noop, ReportTransparentUI = noop,
        },
    },
    ["GameLua.Mod.BaseMod.DS.Security.DSSecurityNotifySystem"] = {
        methods = {
            Notify = noop, SaveInfo = noop, PlayerHasLogin = noop,
        },
    },
    ["GameLua.Mod.BaseMod.GamePlay.AI.FriendlyBehavior"] = {
        methods = {
            ReportFriendlyFire = noop, CheckFriendlyBehavior = noop,
        },
    },
    ["GameLua.Mod.BaseMod.DS.Security.KillFrequencyLimiter"] = {
        methods = {
            CheckKillFrequency = retTrue, ReportKillFrequency = noop,
        },
    },
...
local function applyNetworkBlocker()
    ...
    pcall(function()
        if NetUtil and NetUtil.SendPkg then
            ...
            local blockedPkgs = {
                "battle_client_sync_allstar_auth_check_result_req",
                "hisar",
                "on_crow_update_ntf", "on_crow_update_ntf2", "on_crow_update_ntf3",
                "req_ingame_ban_info",
            }
            NetUtil.SendPkg = function(msg, ...)
                for _, blocked in ipairs(blockedPkgs) do
                    if msg == blocked then return end
                end
                return origSendPkg(msg, ...)
            end
        end
    end)
end
    ...
    if IngamePhoneStateUI and IngamePhoneStateUI.__inner_impl and IngamePhoneStateUI.__inner_impl.UpdateArtQualityUI then
        ...
        IngamePhoneStateUI.__inner_impl.UpdateArtQualityUI = function(self, p1, p2)
            ...
            if self and self.UIRoot and self.UIRoot.TextBlock_quality then
                ...
                self.UIRoot.TextBlock_quality:SetColorAndOpacity(FSlateColor(FLinearColor(0, 0.8, 1, 1)))
            end
        end
    end
end)
...
-- Removed Accessories, Deviation, and Spread to completely fix Fake Damage
local RECOIL_FIELDS = { "RecoilKick", "RecoilKickADS", "AnimationKick", "RecoilModifierStand", "RecoilModifierCrouch", "RecoilModifierProne", "CameraShakeScale", "AimCameraShakeScale", "ShootCameraShakeScale", "FireCameraShakeScale" }

-- Adjusted to server-safe thresholds (Laser visuals, but legit server physics)
local RECOIL_TARGET_VALUES = { 
    RecoilKick = 0.15,           -- 0.45 se seedha 0.25 kar diya (Best balance for damage & stability)
    RecoilKickADS = 0.10,        -- Scope open karne par aur bhi kam
    AnimationKick = 0.02,        -- Gun ka visual kick almost ZERO
    RecoilModifierStand = 0.10,  
    RecoilModifierCrouch = 0.10, 
    RecoilModifierProne = 0.08,  -- Letne par recoil wese hi kam hota hai, isse makkhan ho jayega
    CameraShakeScale = 0.01,      -- Screen shake completely ZERO
    AimCameraShakeScale = 0.01,   
    ShootCameraShakeScale = 0.01, 
    FireCameraShakeScale = 0.01 
}

-- Emptied to prevent strict server-side validation kicks and hit rejections
...
function InjectModMenu()
    ...
    if SettingPageDefine and SettingCatalog and AliasMap and not SettingPageDefine.ModMenu then
        local ModMenuStack = {
            { UI = AliasMap.Title, Text = "✦ GOKU ESP & VISUALS ✦" },
            ...
            { UI = AliasMap.Title, Text = "✦ GOKU GRAPHICS & PERFORMANCE ✦" },
            ...
            { Key = "AimPower", UI = AliasMap.Slider, Text = "Aim Power (0=Low, 100=Max 1.3x)", GetFunc = function() return _G.AimAssist_Power_Slider end, SetFunc = function(_, value) local val = tonumber(value) or 0; if val > 100 then val = 100 end; if val < 0 then val = 0 end; _G.AimAssist_Power_Slider = val; _G.AimAssist_Power = 1.0 + (val / 100) * 0.3; _G.LastAimState = nil; return true end },
...
local function StartMatchFeatures(pc, pawn)
    ...
    pc:AddGameTimer(2.0, true, ThermalGovernorLoop)
    pc:AddGameTimer(300.0, true, AutoRAMCleaner)
...
local function InitializeGokubaBypass()
    pcall(function()
        ...
        if _G.TssManager then
            _G.TssManager.GetUserTag4Lua = function() return "" end
        end
        local gokuba = safe_require("GameLua.Mod.BaseMod.Client.Security.Gokuba")
        if gokuba then
        ...
        if _G.GokubaLogic then
            _G.GokubaLogic.ForwardFeature = function()
                pcall(function()
                    NetUtil.SendPkg("battle_client_sync_allstar_auth_check_result_req", {0, 0, 0, 0, 0})
                end)
            end
        end
        if NetUtil and NetUtil.SendPkg then
            local origSendPkg = NetUtil.SendPkg
            NetUtil.SendPkg = function(msg, ...)
                if msg == "battle_client_sync_allstar_auth_check_result_req" then return end
                return origSendPkg(msg, ...)
            end
        end
    end)
...
-- ==================== NEW: GAMEREPORT / REPLAY TELEMETRY BYPASS ====================
local function InitializeGameReportBypass()
    pcall(function()
        local paths = {
            "GameLua.Mod.BaseMod.GamePlay.GameReport.GameReportUtils",
            "GameLua.Mod.BaseMod.GamePlay.GameReport.GameReportSubsystem",
        }
        for _, path in ipairs(paths) do
            local mod = package.loaded[path]
            if mod then
                mod.ReportException = noop
                mod.BugglyPostExceptionFull = retFalse
                mod.CheckCanBugglyPostException = retFalse
                mod.ReplayReportData = noop
                mod.PostException = noop
                mod.ReportGameException = noop
            end
        end
        pcall(function()
            local pc = slua_GameFrontendHUD:GetPlayerController()
            if slua.isValid(pc) and pc.ClientReplayDataReporter then
                pc.ClientReplayDataReporter.ReportIntArrayData = noop
                pc.ClientReplayDataReporter.ReportUInt8ArrayData = noop
                pc.ClientReplayDataReporter.ReportFloatArrayData = noop
            end
        end)
    end)
end

-- ==================== NEW: DSFIGHTTLOG / SECURITY TLOG BYPASS ====================
local function InitializeDSFightTLogBypass()
    pcall(function()
        local paths = {
            "GameLua.Mod.BaseMod.DS.Security.DSFightTLogSubsystem",
            "GameLua.Mod.BaseMod.DS.Security.DSSecurityTLogSubsystem",
            ...
            "GameLua.Mod.BaseMod.DS.Security.DSAITLogSubsystem",
            "GameLua.Mod.BaseMod.DS.Security.ICTLogSubsystem",
            "GameLua.Mod.BaseMod.DS.Security.DSCreditTLogSubsystem",
        }
        for _, path in ipairs(paths) do
            local mod = package.loaded[path]
            if mod then
                for k, v in pairs(mod) do
                    if type(v) == "function" and (
                        k:find("Report") or k:find("Send") or k:find("Upload") or
                        k:find("Record") or k:find("Flow") or k:find("TLog")
                    ) then
                        pcall(function() mod[k] = noop end)
                    end
                end
            end
        end
...
-- ==================== NEW: AFK BYPASS ====================
local function InitializeAFKBypass()
    pcall(function()
        local paths = {
            "GameLua.Mod.BaseMod.DS.Security.AFKReportorSubsystem",
        ...
        for _, path in ipairs(paths) do
            local mod = package.loaded[path]
            if mod then
                mod.HandleEnterFighting = noop
                mod.InitializePlayerInputInfo = noop
                mod.AddOneAFKInfo = noop
                mod.SetPlayerAFKState = noop
                mod.ResetPlayerInputInfo = noop
                mod.PlayerHaveAction = noop
                mod.ReportAFK = noop
                mod.CheckAFK = retFalse
            end
        end
    end)
end

-- ==================== NEW: CLIENTDATASTATS BYPASS ====================
local function InitializeClientDataStatsBypass()
    pcall(function()
        local mod = package.loaded["GameLua.Mod.BaseMod.Client.Security.ClientDataStatistcsSubsystem"]
        if mod then
            mod.StartToCheck = noop
            mod.OnReceiveRTT = noop
            mod.OnReceiveJitter = noop
            mod.ReportAbnormal = noop
            mod.ResetData = noop
        end
    end)
end
...
pcall(function()
    ...
    safeSelfHeal()
    InitializeGokubaBypass()
    InitializeGameReportBypass()
    InitializeDSFightTLogBypass()
    InitializeAFKBypass()
    InitializeClientDataStatsBypass()
    pcall(function()
...
print("[MOD] ✅ GOKU ULTRA SHIELD – 7-Layer Bypass + All Visuals Applied")
print("[MOD] 🔥 Gokuba/TSS + GameReport + FightTLog + AFK + DataStats Bypass Active")
