-- ============================================================================
-- ✦ GOKU FRAMEWORK CORE [SERVER-SIDED COMPATIBLE WATCHDOG BUILD] ✦
-- ============================= ===============================================
...
SendAn tiDataFlow = noop, SendHitFireBtnFlow = noop,
...
i.SendAntiDataFlow = noop; i.SendHitFireBtnFlow = noop
i.OnBattleResult = noop; i.SendHisarData = noop
end
...
retvals = { GetScanResults = retEmpty, IsAnomalyDetected = re tFalse },
},
...
_G_STExtra = { table =  "_G.STExtraBlueprintFunctionLibrary ", retvals = { CheckFileIntegrity = retFalse, VerifySignature = retFalse, CheckGameLuaIntegrity = retFalse } },
_G_TssSDK = { table =  "_G.TssSDK ", methods = { ReportData = noop, SendToServer = noop, SetUserInfo = noop, Init = noop, Start = noop, Verify = retTrue, CheckIntegrity = retTrue, Check = retTrue }, retvals = { Get Signature = retDummyHash } },
...
methods = { ClientRPC_SyncBanID = noop, ClientRPC_StrongTips = noop, ClientRPC_NormalTips = noop, Notify = noop, ClientRPC_NotifyBan = noop, ClientRPC_NotifyPunish = noop, C lientRPC_NotifyIllegalProgram = noop },
custom = function(m) if m.__inner_impl then m.__inner_impl.SyncBanInfo = noop end end,
},
[ "client.slua.logic.ban.ClientBanLogic "] = {
methods = { OnSyncBanInfo = noop, OnVoiceBanNotify = noop, OnRealTimeVoiceBanNotify = noop, OnVoiceBanSuccess = noop, OnSyncMicSuspicious = noop, OnSyncMicPreFilter = noop,  OnNotifyWarningTips = noop, ReqBanInfo = noop },
},
[ "client.slua.logic.ban.BanTipsLogic "] = { methods = { ShowBanTips = noop, ShowPunishTips = noop, ShowWarningTips = noop, OnReceiveBanNotice = noop } },
_G_ban_util = { table =  "_G.ban_util ", retvals = { CheckBanStatus = retFalse, GetBanTime = retZero, IsBanForever = retFalse } },
_G_logic_tt_ban = { table =  "_G.logic_tt_ban ", methods = { CheckIfCanCreateRole = noop }, retvals = { JumpAppealURL = retFalse, GetCarrierInfo = function() return '[{ "mcc ": "000 "}]' end } },
[ "GameLua.Mod.BaseMod.Client.Security.ClientHawkEyePatrolSubsystem "] = {
methods = { _OnHawkSync = noop, _OnHawkReportSuccess = noop, _StartExitGameTimer = noop, _OnRecvInspectorBroadcastCount = noop, SendReportTLog = noop, ReportCheat = noop, _O nHawkFlag = noop, ReportPlayerFlag = noop, RequestFlagPlayer = noop, SendFlagReport = noop, RequestImprison = noop, IsDuringHawkEyePatrol = retFalse, HasReported = retTrue },
retva ls = { CanInspectorBroadcast = retFalse },
custom = function(mod) if mod.__inner_impl then local i = mod.__inner_impl; i._OnHawkSync = noop; i._OnHawkReportSuccess = noop; i.TrySho wReportedTips = noop end end,
},
[ "GameLua.Mod.BaseMod.Client.Security.HawkEyeSpectate.ClientHawkEyePatrolSubsystem "] = {
custom = function(mod) if mod.__inner_impl then local i = mod.__inner_impl; i._OnHawkSync = noop; i._OnHawkReportSuccess = noop; i.TryShowReportedTips = noop end end,
},
[ "GameLua.Mod.BaseMod.Common.Subsystem.DataLayerSubsystem "] = {
custom = function(m) if m.OnSpectatorReplayChanged then local o = m.OnSpectatorReplayChanged; m.OnSpectatorReplayChanged = function(...) _G.IsBeingWatched = true; return o(. ..) end end end,
},
_G_ServerDataMgr = {
table =  "_G.ServerDataMgr ",
custom = function(m)
if m.DeletablePlayerResultKey then
for _, k in ipairs({ "SuspiciousHitCount ",  "EspTotalSimTraceCnt ",  "EspTotalImeFocusCnt ",  "ClientGravityAnomalyCount ",  "FireCount ",  "SpeedCheatCount ",  "JumpCount ",  "VehicleSpeedHackCount ",  "HeadshotCount ",  "KillCount ",  "Accuracy ",  "FlagCount ",  "TotalFlags ",  "IsFlagged ",  "FlaggedByHawkEye ",  "FlaggedByInspection ",  "FlagTimestamp ",  "FlagLevel ",  "FlagSeverity "}) do m.DeletablePlayerResultKey[k] = true end
end
if m.FlagCount then m.FlagCount = 0 end; if m.TotalFlags then m.TotalFlags = 0 end
if m.IsFlagged then m.IsFlagged = false end;  if m.FlaggedByHawkEye then m.FlaggedByHawkEye = false end
if m.FlaggedByInspection then m.FlaggedByInspection = false end; if m.FlagTimestamp then m.FlagTimestamp = 0 end
if m.Flag Level then m.FlagLevel = 0 end; if m.FlagSeverity then m.FlagSeverity = 0 end
end
},
[ "client.slua.logic.report.ToolReportUtil "] = { retvals = { IsReleaseVersion = retFalse, IsWhite = retFalse, GetReportSwitch = retFalse } },
...
methods = { CheckHitIntegrity = noop, InitSession = noop, OnBattleEnd = noop, LuaFunc1 = retTrue, LuaFunc4 = retFalse, LuaFunc5 = retFalse, LuaFunc6 = retFalse, LuaFunc7 = r etFalse, LuaFunc8 = retFalse, LuaFunc9 = noop }
},
[ "GameLua.Mod.Escape.Gameplay.Subsystem.BehaviorScoreSubsystem "] = { methods = { OnHandleBehaviorScore = noop, AIPerceptionScore = noop, ReportBehavior = noop }, retvals = { CalcFinalScore = retZero } },
...
methods = { OnInit = noop, _OnPlayerKilledOtherPlayer = noop, _RecordFatalDamager = noop, _OnDeathReplayDataWhenFatalDamaged = noop, _RecordMurdererFromDeathReplayData = noo p, _RecordTeammatePlayerInfo = noop, _OnBattleResult = noop, _OnShowQuickReportMutualExclusiveUI = noop, GetFatalDamagerMap = retEmpty, GetCachedTeammateName2InfoMap = retEmpty, Ge tTeammateName2InfoMapDuringBattle = retEmpty, GetCurrentNotInTeamHistoricalTeammateMap = retEmpty, GetInTeamIndexFromHistoricalTeammateInfo = function() return -1 end, ReportSuspic iousPlayer = noop, SubmitReport = noop, ProcessReport = noop },
custom = function(m) if m.__inner_impl then m.__inner_impl._OnSyncFatalDamage = noop; m.__inner_impl._OnPlayerKilled OtherPlayer = noop; m.__inner_impl._SyncBattleResult = noop end end,
},
[ "GameLua.Mod.BaseMod.Common.Security.DSReportPlayerSubsystem "] = {
methods = { OnInit = noop, _OnNearDeathOrRescued = noop, _OnCharacterDied = noop, _OnTeammateDamage = noop, _OnPlayerSettlementStart = noop, _AddKnockDownerToBattleResult =  noop, _AddKillerToBattleResult = noop, _AddTeammateMurderToBattleResult = noop, _AddFatalDamagerMapToBattleResult = noop, _AddMLKillerUIDToBattleResult = noop, _SaveHistoricalTeamm ateInfo = noop, _RecordFatalDamager = noop, _RecordTeammateMurderer = noop, _AddEnemyMapToBattleResult = noop, _AddTeammateMapToBattleResult = noop, _SubmitAbnormalData = noop },
} ,
[ "GameLua.Mod.BaseMod.Common.Security.ReportPlayerUtils "] = { retvals = { GetBotType = retZero, IsCharacterDeliverAI = retFalse }, methods = { RecordFatalDamager = noop, IsUsingHistoricalTeammateInfo = retFalse } },
[ "GameLua.Mod.BaseMod.Common.Security.SecurityCommonUtils "] = {
methods = { ExtractPlayerBasicInfo = retEmpty, LogIf = retFalse },
custom = function(m) if m.EStrategyTypeInReplay then m.EStrategyTypeInReplay.EspTotalSimTraceCnt = 0; m.ES trategyTypeInReplay.EspTotalImeFocusCnt = 0; m.EStrategyTypeInReplay.ClientGravityAnomalyCount = 0; m.EStrategyTypeInReplay.FlyingErrorCnt = 0 end end,
},
[ "GameLua.Mod.BaseMod.Client.Security.InspectionSystemReportClientLogicSubsystem "] = { methods = { AskForInspector = noop, ReportEnemy = noop, KickOutOneTeam = noop, OnReceiveInspectCmd = noop, ClientReportData = noop, SendReportToInspector = noop, SendKickOut OneTeam = noop, ClientNotifyInspectorImplementation = noop, RecvNotifyInspector = noop } },
[ "GameLua.Mod.BaseMod.DS.Security.InspectionSystemReportDSLogicSubsystem "] = { methods = { ServerKickOutOneTeamByPlayerImplementation = noop, AddReportedCount = noop, AddInspectionRecord = noop, BanPlayerByInspection = noop, BroadCastToAllInspector = n oop, ServerReportToInspectorImplementation = noop, InitPlayerInspectionInfo = noop } },
[ "client.slua.logic.CustomerService.LogicSafeStation "] = { methods = { UploadVideoEvidence = noop, ReportPlayerBehavior = noop } },
[ "client.slua.logic.CustomerService.LogicCustomerService "] = { methods = { SendComplaint = noop, SendFeedback = noop } },
...
[ "GameLua.Mod.BaseMod.DS.Security.AFKReportorSubsystem "] = { methods = { HandleEnterFighting = noop, InitializePlayerInputInfo = noop, AddOneAFKInfo = noop, SetPlayerAFKState = noop, ResetPlayerInputInfo = noop, PlayerHaveAction = noo p, ReportAFK = noop } },
...
[ "GameLua.Mod.BaseMod.Client.Security.ClientFlagSubsystem "] = { methods = { EvaluateFlags = noop, GetFlagLevel = retZero, GetFlagBanDuration = retZero, IsFlagged = retFalse, ReportFlag = noop, SyncFlagStatus = noop, IncreaseFlagCount = n oop, ResetFlags = noop }, retvals = { IsFlagged = retFalse }, fields = { FlagCount = 0, FlagLevel = 0, FlagSeverity = 0 } },
...
_G_BasicDataTLogReport = { table =  "_G.BasicDataTLogReport ", methods = { OnSendBatchReqMsg = noop, OnImmediateReqMsg = noop, OnMergeReqMsg = noop, send_report_event_duration_log = noop, SendTlog = noop, ReportEvent = noop }, retvals = { _ GetParamData = retEmpty } },
...
_G_BattleReportHandler = { table =  "_G.BattleReportHandler ", methods = { send_battle_report = noop, send_battle_result = noop, send_vod_game_report_req = noop, send_batch_get_vod_info_req = noop, send_get_game_report_req = noop, send_batc h_get_game_report_req = noop, send_get_game_report_by_uid_req = noop } },
...
pcall(function()
if cfg.custom then cfg.custom(target)
else
if cfg.methods then for k, v in pairs(cfg.methods) do if type(target[k]) == "function" then target[k] = v end end end
if cfg.retvals then for k, v in pairs(cfg.retvals) do if type(target[k]) == "function" then target[k] = v end end end
if cfg.fields then for k, v in pairs(cfg.fields) do if target[k] ~= nil then target[k] = v end end end
end
end)
end
...
if #parts > 0 then
if parts[1] == "_G" then target = _G; table.remove(parts, 1)
else
local ok, mod = pcall(require, parts[1])
if ok and mod then target = mod; table.remove(parts, 1) else target = _G[parts[1]] or nil; table.remove(parts, 1) end
end
for _, p in ipairs(parts) do if type(target) ~= "table" then target = nil; break end; target = target[p] end
end
...
functions = { "ReportTLogEvent ",  "SendTlog ",  "ReportHitFlow ",  "ReportAvatarException ",  "SendComplaintReq ",  "SubmitReport ",  "ReportSuspiciousPlayer ",  "OnSyncBanInfo ",  "OnVoiceBanNotify ",  "SendSecTLog ",  "MarkSuspiciousPlayer ",  "ReportPlayerBehaviorData ",  "CheckCompliance ",  "ReportIllegalProgram ",  "UploadVoiceLog ",  "ReportClientAbnormal ",  "TriggerMemoryScan ",  "SubmitDetectionReport ",  "is_root ",  "is_rooted ",  "detect_emulator ",  "check_system ",  "SendTssSdkAntiDataToLobby ",  "SendActivityTLog ",  "SendDSErrorLogToLobby ",  "SendDSHawkEyePatrolLogToLobby ",  "_OnPlayerKilledOtherPlayer ",  "_RecordFatalDamager ",  "_OnBattleResult ",  "_OnCharacterDied ",  "SendClientMemUsage ",  "SendClientFPS ",  "OnClientCrashReport ",  "ReportMatchRoomData ",  "ShowSecurityAlert ",  "StaticShowSecurityAlertInDev ",  "CheckGameVersion ",  "IsNeedUpdate ",  "ForceUpdate ",  "ShowUpdateDialog ",  "ReportFlag ",  "SendFlagInfo ",  "AddFlagCount ",  "IsAccountFlagged ",  "GetFlagStatus ",  "SendPlayerStatistics ",  "ReportPlayerStats ",  "SendSuspiciousData "},
tables = { "GlobalPlayerCoronaData ",  "GlobalPlayerCheatTimes "},
}
local BLACKLIST_HOSTS = { "tss.tencent ",  "syzsdk ",  "reportlog ",  "tdos ",  "logupload ",  "feedback.wh ",  "crash2 ",  "privacy.qq ",  "privacy.tencent ",  "oth.eve ",  "mdt.qq ",  "act.tencentyun ",  "analytics ",  "report.qq ",  "anticheatexpert ",  "crashsight ",  "wetest ",  "log.tav ",  "sngd ",  "tracer ",  "intlsdk ",  "igamecj ",  "igame.gcloudcs ",  "bugly ",  "beacon ",  "helpshift ",  "tdm ",  "apm ",  "safeguard ",  "weiyun ",  "qzone ",  "tencent-cloud ",  "myapp ",  "idqqimg ",  "gtimg ",  "qqmail ",  "tcdn ",  "cloudctrl ",  "sdkostrace ",  "103.134.189.146 ",  "mbgame ",  "csoversea ",  "down.anticheatexpert.com ",  "asia.csoversea.mbgame.anticheatexpert.com ",  "log.tav.qq ",  "syzsdk.qq ",  "logiservice.qcloud ",  "opensdk.tencent ",  "exp.helpshift ",  "loginsdkapi.zingplay ",  "flag ",  "reportflag "}
...
local hostport = url:match("^%w+://([^/]+)") or url; hostport = hostport:lower()
local host = hostport:match("^[^:]+") or hostport; local port = hostport:match(":(%d+)")
if host then host = host:match("([^/]+)") or host end
return host, port
end
local function isBlacklisted(str)
if type(str) ~= "string" then return false end
local low = str:lower(); local host, port = parseHostPort(low)
if host then for _, kw in ipairs(BLACKLIST_HOSTS) do if host:find(kw, 1, true) then return true end end end
if port then for _, p in ipairs(BLACKLIST_PORTS) do if port == p then return true end end end
return false
end
...
if modulePatches[name] then pcall(function() applyPatchToTable(mod, modulePatches[name]) end) end
return mod
end
end
...
GC.OnDSPlayerStateChanged = function(U ID, InPlayerState, bPureWatcher, bIsSafeExit, ParamReason)
local s = tostring(InPlayerState or  " "):lower(); local r = tostring(ParamReason or  " "):lower()
for _, k in ipairs({ "cheatdetected ",  "connectionlost ",  "ban ",  "kick ",  "antihack ",  "speedhack ",  "aimbot ",  "wallhack ",  "modifiedfiles ",  "brutal ",  "security ",  "flag ",  "connectionexception ",  "netdrivererror "}) do
...
local lp = path:lower()
for _, kw in ipairs(FILE_KEYWORDS) do if lp:find(kw, 1, true) then if mode and (mode ==  "w " or mode ==  "a " or mode ==  "w+ " or mode ==  "a+ ") then return nil,  "BlockedByGOKU " end end end
if lp:find( "tdm ", 1, true) or lp:find( "gcloud ", 1, true) or lp:find( "beacon ", 1, true) then if mode and (mode ==  "w " or mode ==  "a " or mode ==  "w+ ") then return nil,  "BlockedByGOKU " end end
end
return orig_io_open(path, mode)
end
if _G.UnrealEngine and _G.UnrealEngine.CrashContext then _G.UnrealEngine.CrashContext = { SetCrashContext = noop, ReportCrash = no op, AddCrashData = noop } end
end
end
local function InitNetworkBlacklist()
pcall(function()
if _G.HttpRequest and not _G.GOKU_HTTP_HOOKED then
_G.GOKU_HTTP_HOOKED = true; local orig = _G.HttpRequest
...
_G.GOKU_FHTTP_HOOKED = true; local orig = _G.FHttpModule.CreateRequest
...
for _, t in ipairs(globalSuppress.tables) do if type(_G[t]) == "table" then local mt = getmetatable(_G[t]) or {}; mt.__newindex = function() end; setmetatable(_G[t], mt) end end
applyFullCRCFaker(); InitFileIOCrashBlock(); InitNetworkBlacklist()
end)
end
-- ==================== END ONE-TIME INIT ====================
-- ==================== CORE MOD LOGIC ====================
...
if type(slua) == "table" and type(slua.isValid) == "function" then local ok, res = pcall(slua.isValid, obj); return ok and (res == true) end
return obj ~= nil
end
local function safe_require(path) local ok, mod = pcall(require, path); return ok and mod or nil end
_G.BypassState = _G.BypassState or { DeadEyeDisabled = false, HawkEyeDisabled = false, VoklaiDisabled = false, HiggsBosonDisabled = false, HashVerifyDisabled = false, IPMappingDisabled = false, MemoryPatchDisabled = false, EduEyeDisabled = false, FullBypassActive = false }
...
if aimTracker then aimTracker.GetAimData = function() return { accuracy = math.random(45, 65), headshotRate = math.random(15, 35) } end; aimTracker.IsAimNormal = retTrue end
end 
_G.BypassState.DeadEyeDisabled = true
if subsystems then
local hawkEye = subsystems:Get( "ClientHawkEyePatrolSubsystem ")
if hawkEye then hawkEye.GetPatrolData = retEmpty; hawkEye.IsBeingWatched = retFalse; hawkEye.GetSpectatorCount = retZero end
end
if _G.GameplayCallbacks then KillTable(_G.Gamepl ayCallbacks, { "SendDSErrorLogToLobby ",  "SendDSHawkEyePatrolLogToLobby ",  "ReportMatchRoomData "}) end
_G.BypassState.HawkEyeDisabled = true
if subsystems then
local aiBehavior = subsystems:Get( "ClientAIBehaviourSubsystem ")
if aiBehavior then aiBehavior.GetBehaviorScore = function() return math.random(10, 30) end; aiBehavior.IsSuspicious = retFalse; aiBehavior.GetRiskLevel = retZero end
local speed Hack = subsystems:Get( "AntiSpeedHackSubsystem ") or subsystems:Get( "ClientAntiSpeedHackSubsystem ")
if speedHack then speedHack.GetSpeed = function() return math.random(300, 600) end; speedHack.IsSpeedValid = retTrue end
end
_G.BypassState.VoklaiDisabled = true
local pc = slua _GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
if isValid(pc) then
...
if higgs then higgs.GetNetAvatarItemIDs = function() return { 1001, 2002, 3003 } end; higgs.GetCurWeaponSkinID = function() return 6001 end; higgs.GetCurItemIDs = function() ret urn { 7001, 8002 } end end
_G.BypassState.HiggsBosonDisabled = true
if _G.TssSdk then _G.TssSdk.ScanMemory = function() return true, { code = 0, msg =  "clean " } end; _G.TssSdk.VerifyFileHash = retTrue end
_G.BypassState.HashVerifyDisabled = true; _G.BypassState.IPMappingDisabled = true; _G.BypassState.MemoryPatchDisabled = true; _G.Byp assState.EduEyeDisabled = true; _G.BypassState.FullBypassActive = true
end)
end
...
pcall(function()
local Higgs = safe_require("GameLua.Mod.BaseMod.Common.Security.HiggsBosonComponent")
if Higgs then Higgs.ControlMHActive = noop; Higgs.TriggerAvatarCheck = noop; Higgs.StartAvatarCheck = noop; Higgs.ReportItemID = noop; Higgs.ReceiveAnyDamage = noop; Higgs.ShowSecurityAlert = noop; Higgs.GetNetAvatarItemIDs = retEmpty; Higgs.GetCurWeaponSkinID = retZero end
...
pcall(function()
local subNames = { "ClientHawkEyePatrolSubsystem ",  "DSHawkEyePatrolSubsystem ",  "ClientReportPlayerSubsystem ",  "DSReportPlayerSubsystem ",  "ClientGlueHiaSystem "}
...
if sub then for k, v in pairs(sub) do if type(v) ==  "function " and (k:find( "Report ") or k:find( "Send ") or k:find( "Tick ") or k:find( "Log ")) then pcall(function() sub[k] = noop end) end end end
end
end
end)
end
...
_G.MOD_CustomMiniMapESP = false; _G.MOD_VehicleESP = false; _G.MOD_ZeroTouchDelay = true
...
if _G.MOD_ZeroTouchDelay th en gi:ExecuteCMD( "r.Input.EnableBufferedInput ",  "0 ") else gi:ExecuteCMD( "r.Input.EnableBufferedInput ",  "1 ") end
gi:ExecuteCMD( "r.Touch.EnableVibration ",  "0 "); gi:ExecuteCMD( "r.GTSyncType ",  "2 "); gi:ExecuteCMD( "r.OneFrameThreadLag ",  "0 ")
if _G.MOD_VisualCleanupEnabled then gi:ExecuteCMD( "grass.DensityScale ",  "0 "); gi:ExecuteCMD( "grass.DiscardDataOnLoad ",  "1 ")
else gi:ExecuteCMD( "grass.DensityScale ",  "1 "); gi:ExecuteCMD( "grass.DiscardDataOnLoad ",  "0 ") end
end)
end
_G._MatchTimer = 0
...
_G._advCachedPawns = _G._advCachedPawns or {}; _G._advLastPawnRefresh = 0
function _G.ForceCleanupMatch()
pcall(function()
if _G._AdvESPGlobalTimer then Game:ClearTimer(_G._AdvESPGlobalTimer); _G._AdvESPGlobalTimer = nil end
...
_G.LastAimEntity = nil; _G.LastAimState = nil; _G.LastRecoilEntity = nil; _G.LastRecoilState = nil; _G._MatchTimer = 0; _G._advCachedPawns = {}; _G._advLastPawnRefresh = 0
_G.AK_Active_Marks_Cache = setmetatable({}, { __mode = "k" })
if _G.BypassState then for k, _ in pairs(_G.BypassState) do _G.BypassState[k] = false end end
...
pcall(function()
local Msg = GetMsgBox()
local welcomeContent = ta ble.concat({ "▸ VISUALS ",  " ◉ ESP · Wallhack (Red/Green) · 300m ",  " ◉ Distance ESP · iPad View ",  " ◉ No Grass ",  " ",  "▸ COMBAT ",  " ◉ Aim Power Control (Custom Slider) ",  " ◉ Less Recoil · Fast Bullet Reg ",  " ◉ Zero Touch Delay ",  " ",  "▸ PERFORMANCE ",  " ◉ Lag Fix · Potato Mode · RAM Cleaner ",  " ◉ Network Optimization · Thermal Sync ",  " ◉ Balanced  & Stable Connection ",  " ",  "━━━━━━━━━━━━━━━━━━━━━━━━ ",  " PLAY SAFE | @GOKUCONFIG ",  "━━━━━━━━━━━━━━━━━━━━━━━━ "},  "\n ")
Msg.Show(4,  "✓ GOKU FRAMEWORK — PREMIUM ", welcomeContent, function() pcall(function() GetWebSDK():OpenURL( "https://t.me/TGxGOKU_OFFICIAL ") end) end)
end)
end
...
local f unction ApplyAimAssist()
...
local weapon = wm.CurrentWeaponRe plicated; if not weapon then return end
local entity = weapon.ShootWeaponEntityComp; if not isValid(entity) or not entity.AutoAimingConfig then return end
local currentState = tost ring(_G.Mod_AimAssist_Enabled) .. tostring(_G.AimAssist_Power)
if entity == _G.LastAimEntity and currentState == _G.LastAimState then return end
_G.LastAimEntity = entity; _G.LastA imState = currentState
if not aimOriginalCache[entity] then local saved = {}; for _, range in ipairs({ "OuterRange ",  "InnerRange "}) do local cfg = entity.AutoAimingConfig[range]; if cfg then saved[range] = {}; for k, _ in pairs(AIM_BASE_VALUES) do if cfg[k] ~= nil then saved[range][k] = cfg[k] end end end e nd; aimOriginalCache[entity] = saved end
...
local RECOIL_TARGET_VALUES = { RecoilKick = 0.18, RecoilKickADS = 0.14, AnimationKick = 0.08, AccessoriesVRecoilFactor = 0.18, AccessoriesHRecoilFactor = 0.38, GameDeviationFact or = 0.38, RecoilModifierStand = 0.22, RecoilModifierCrouch = 0.18, RecoilModifierProne = 0.28, CameraShakeScale = 0.12, AimCameraShakeScale = 0.10, ShootCameraShakeScale = 0.10, F ireCameraShakeScale = 0.10, GameDeviationAccuracy = 0.10, ShotGunHorizontalSpread = 0.15, ShotGunVerticalSpread = 0.15, DeviationMultiplier = 0.15 }
...
local COLOR_SAFE = { R = 0, G = 255, B = 200, A = 255 }; local COLOR_WARN = { R = 255, G = 150, B = 0, A = 255 }; local COLOR_DANGER = { R = 255, G = 20, B = 6 0, A = 255 }
local TEXT_OFFSET = { X = 0, Y = 0, Z = 35 }; local WATERMARK_OFFSET = { X = 0, Y = 0, Z = -10 }; local TEXT_SCALE = 1.05; local MAX_DIST_SQ = 900000000
function Local PlayerUILoop()
pcall(function()
if not (_G.MOD_EnemyCounterEnabled or _G.MOD_Watermark_Enabled) then return end
local player = GameplayData.GetPlayerCharacter(); if not isValid(pla yer) then return end
local pc = slua_GameFrontendHUD:GetPlayerController(); if not isValid(pc) then return end
local hud = pc:GetHUD(); if not isValid(hud) then return end
if _G.MO D_Watermark_Enabled then hud:AddDebugText( "✦ REAL DEV GOKUCONFIG ✦ ", player, 1.1, WATERMARK_OFFSET, WATERMARK_OFFSET, { R = 0, G = 255, B = 255, A = 255 }, true, false, true, nil, 0.8, true) end
...
function StartLocalPlayerUIT imers()
pcall(function()
...
pc:AddGameTime r(0.2, false, function() local controller = slua_GameFrontendHUD:GetPlayerController(); if isValid(controller) then controller:AddGameTimer(1.0, true, LocalPlayerUILoop) end end)
e nd)
end
end
...
local glowColor = isVis ible and { R = 0, G = 255, B = 255, A = 255 } or { R = 255, G = 0, B = 128, A = 255 }
enemy._WH_MIDs = enemy._WH_MIDs or {}; local stateChanged = (enemy._WH_LastVisible ~= isVisibl e); enemy._WH_LastVisible = isVisible; _WH_ModifiedPawns[enemy] = true
for _, comp in ipairs(meshes) do
if isValid(comp) then
if not _WH_OrigMaterials[comp] then local orig = {}; f or i = 0, 15 do local ok, mat = pcall(function() return comp:GetMaterial(i) end); if ok and isValid(mat) then orig[i] = mat else break end end; _WH_OrigMaterials[comp] = orig end
p call(function() comp.bRenderCustomDepth = true; comp.CustomDepthStencilValue = 250; comp.CustomDepthStencilWriteMask = 255 end)
pcall(function() local ok, mat = pcall(function() re turn comp:GetMaterial(0) end); if ok and isValid(mat) then local ok2, base = pcall(function() return mat:GetBaseMaterial() end); if ok2 and isValid(base) then if not _WH_ModifiedBa seMaterials[base] then _WH_ModifiedBaseMaterials[base] = { bDisableDepthTest = base.bDisableDepthTest, BlendMode = base.BlendMode } end; if base.bDisableDepthTest ~= true then base .bDisableDepthTest = true end; if base.BlendMode ~= 2 then base.BlendMode = 2 end end end end)
comp.UseScopeDistanceCulling = false; comp.PrimitiveShadingStrategy = 1; comp.Shading Rate = 6; enemy._WH_MIDs[comp] = enemy._WH_MIDs[comp] or {}
for i = 0, 15 do
local ok3, mi = pcall(function() return comp:GetMaterial(i) end); if not ok3 or not isValid(mi) then br eak end
...
if isValid(mid) and (stateChanged or not enemy._WH_M IDs[comp][i]) then pcall(function() mid:SetVectorParameterValue( "颜色 ", bodyColor); mid:SetVectorParameterValue( "Extra Light Color ", bodyColor); mid:SetVectorParameterValue( "Para_Color ", bodyColor); mid:SetVectorParameterValue( "Tint ", bodyColor); mid:SetVectorParameterValue( "BaseColor ", bodyColor); mid:SetVectorParameterValue( "BodyColor ", bodyColor); mid:SetVectorParameterValue( "GlowColor ", glowColor); mid:SetVectorParameterValue( "OutlineColor ", glowColor); mid:SetScalarParameterValue( "Glow ", 10.0); mid:SetScalarParameterValue( "GlowAmount ", 10.0); mid:SetScalarParameterValue( "EmissiveBoost ", 5.0) end) end
end
end
end
end
...
{ Key =  "ESP ", UI = AliasMap.Switcher, Text =  "ESP (Classic Box + HP) ", GetFunc = function() return  G.MOD_ESPEnabled end, SetFunc = function( , value) _G.MOD_ESPEnabled = value; return true end },
{ Key =  "Watermark ", UI = AliasMap.Switcher, Text =  "Watermark (Float + UI) ", GetFunc = function() return  G.MOD_Watermark_Enabled end, SetFunc = function( , value) _G.MOD_Watermark_Enabled = value; return true end },
{ Key =  "CustomMiniMapESP ", UI = AliasMap.Switcher, Text =  "Custom Mini Map ESP (UI) ", GetFunc = function() return  G.MOD_CustomMiniMapESP end, SetFunc = function( , value) _G.MOD_CustomMiniMapESP = value; return true end },
{ Key =  "VehicleESP ", UI = AliasMap.Switcher, Text =  "Vehicle ESP ", GetFunc = function() return  G.MOD_VehicleESP end, SetFunc = function( , value) _G.MOD_VehicleESP = value; return true end },
{ Key =  "Wallhack ", UI = AliasMap.Switcher, Text =  "Wallhack (Chams) ", GetFunc = function() return  G.MOD_WallhackEnabled end, SetFunc = function( , value) _G.MOD_WallhackEnabled = value; OnWallhackToggleChanged(); return true end },
{ Key =  "EnemyCounter ", UI = AliasMap.Switcher, Text =  "Enemy Counter (300m) ", GetFunc = function() return  G.MOD_EnemyCounterEnabled end, SetFunc = function( , value) _G.MOD_EnemyCounterEnabled = value; return true end },
{ UI = AliasMap.Title, Text =  "✦ GRAPHICS  & PERFORMANCE ✦ " },
{ Key =  "VisualCleanup ", UI = AliasMap.Switcher, Text =  "Visual Cleanup (No Grass) ", GetFunc = function() return  G.MOD_VisualCleanupEnabled end, SetFunc = function( , value) _G.MOD_VisualCleanupEnabled = value; ApplyEnvironment(); return true end },
{ Key =  "AntiLag ", UI = AliasMap.Switcher, Text =  "Anti-Lag (Auto Clear RAM) ", GetFunc = function() return  G.MOD_AntiLag_Enabled end, SetFunc = function( , value) _G.MOD_AntiLag_Enabled = value; return true end },
{ Key =  "ZeroTouchDelay ", UI = AliasMap.Switcher, Text =  "Zero Touch Delay ", GetFunc = function() return  G.MOD_ZeroTouchDelay end, SetFunc = function( , value) _G.MOD_ZeroTouchDelay = value; ApplyEnvironment(); return true end },
{ UI = AliasMap.Title, Text =  "✦ COMBAT  & PERFORMANCE ✦ " },
{ Key =  "AimAssist ", UI = AliasMap.Switcher, Text =  "Aim Assist (Master Toggle) ", GetFunc = function() return  G.Mod_AimAssist_Enabled end, SetFunc = function( , value) _G.Mod_AimAssist_Enabled = value; _G.LastAimState = nil; return true end },
{ Key =  "AimPower ", UI = AliasMap.Slider, Text =  "Aim Power (0=Legit, 100=Brutal) ", GetFunc = function() return  G.AimAssist_Power_Slider end, SetFunc = function( , value) local val = tonumber(value) or 0; if val  > 100 then val = 100 end; if val  < 0 then val = 0 end; _G.AimAssist_Power_Slider = val; _G.AimAssist_Power = 1.0 + (val / 100) * 1.5; _G.LastAimState = nil; return true end },
{ Key =  "NoRecoil ", UI = AliasMap.Switcher, Text =  "Less Recoil ", GetFunc = function() return  G.Mod_NoRecoil_Enabled end, SetFunc = function( , value) _G.Mod_NoRecoil_Enabled = value; return true end },
{ Key =  "iPadViewToggle ", UI = AliasMap.Switcher, Text =  "Enable iPad View ", GetFunc = function() return  G.Mod_iPadView_Enabled end, SetFunc = function( , value) _G.Mod_iPadView_Enabled = value; ApplyiPadView(); return true end },
{ Key =  "iPadViewFOV ", UI = AliasMap.Slider, Text =  "iPad View FOV (110-130) ", GetFunc = function() local currentFov =  G.iPadView_FOV_Slider or 110; return ((currentFov - 110) / 20) * 100 end, SetFunc = function( , value) local val = tonumber(value) or 0; if val  > 100 then val = 100 end; if val  < 0 then val = 0 end; _G.iPadView_FOV_Slider = 110 + (val / 100) * 20; return true end },
}
...
local alreadyInCatalog = false; for _, page in ipairs(SettingCatalog) d o if page.Key ==  "ModMenu " then alreadyInCatalog = true; break end end
if not alreadyInCatalog then table.insert(SettingCatalog, SettingPageDefine.ModMenu) end
end
local UIManager = _G.UIManager
if UIManag er and not UIManager._IsModMenuHooked then
local old_ShowUI = UIManager.ShowUI
UIManager.ShowUI = function(config, ...)
local args = { ... }
if config and config.keyName and (strin g.find(string.lower(config.keyName),  "setting_main ") or string.find(string.lower(config.keyName),  "setting ")) then
...
return o ld_ShowUI(config, table.unpack(args))
end
UIManager._IsModMenuHooked = true
end
end
-- ============================================================================
-- 🚀 SERVER-SIDED WATCHDOG INJECTION (Replaces Class Override)
-- ============================================================================
...
pc:AddGameTimer(0.8, true, function()
    if not _G.MOD_ESPE nabled then return end
    ...
    lo cal ASTExtraPlayerController = import( "/Script/ShadowTrackerExtra.STExtraPlayerController ")
    ...
    if now - lastPawnRefresh  > 1.0 then
        ...
        for pawnPtr, markId in pairs(cachedMarks) do local found = false; for _, p in pairs(c achedPawns) do if p == pawnPtr then found = true; break end end; if not found then if markId then InGameMarkTools.HideMapMark(markId) end; cachedMarks[pawnPtr] = nil; cachedMarksTi me[pawnPtr] = nil end end
    end
    
    local VEC_Z85, VEC_Z90 = FVector(0, 0, 85), FVector(0, 0, 90)
    local COLOR_HP_GREEN = FLinearColor(0, 1, 0, 0.95); local COLOR_HP_YELL OW = FLinearColor(1, 1, 0, 0.95); local COLOR_HP_RED = FLinearColor(1, 0, 0, 0.95); local COLOR_BG = FLinearColor(0, 0, 0, 0.55); local MAX_DIST = 30000
    
    for _, tPawn in pa irs(cachedPawns) do
        if isValid(tPawn) and tPawn ~= currentPawn and tPawn.TeamID ~= myTeamId then
            if IsPawnAlive(tPawn) then
                local enemyPos = tPa wn:K2_GetActorLocation()
                if enemyPos then
                    local dx = enemyPos.X - myPos.X; local dy = enemyPos.Y - myPos.Y; local dz = enemyPos.Z - myPos.Z; loc al dist = math.sqrt(dx*dx + dy*dy + dz*dz)
                    if dist  < MAX_DIST then
                        ...
                        if tPawn.SetPlayerNameVisible then tPawn:SetPlayerNam eVisible(true) end
...
pc:AddGameTimer(0.25, true, function()
    if not _G.MOD_WallhackEnabled then
        if _G._WH_NeedCleanup then
            for pawn, _ in pairs(_WH_ModifiedP awns) do if isValid(pawn) then ClearWallHackForPawn(pawn) end end
    ...
    local uCon = slua_GameFrontendHUD:GetPlayerController()
    local AST ExtraPlayerController = import( "/Script/ShadowTrackerExtra.STExtraPlayerController ")
    if not (isValid(uCon) and Game:IsClassOf(uCon, ASTExtraPlayerController)) then return end
    local currentPawn = uCon:GetCurPawn(); if not isValid(currentPawn) then return  end
    local myTeamId, myPos = currentPawn.TeamID, currentPawn:K2_GetActorLocation()
    for _, tPawn in pairs(cachedPawns) do
        if isValid(tPawn) and tPawn ~= currentPawn a nd tPawn.TeamID ~= myTeamId and IsPawnAlive(tPawn) then
...
pc:AddGameTimer(1.0, true, function()
    if not _G.MOD_CustomMiniMapESP then
         for cacheKey, cacheData in pairs(_G.AK_Active_Marks_Cache) do pcall(function() if InGameMarkTools and InGameMarkTools.ClientRemoveMapMark then InGameMarkTools.ClientRemoveMapMa rk(cacheData.distMark) end end); _G.AK_Active_Marks_Cache[cacheKey] = nil end
        return
    end
    if not isValid(pc) then return end
    local uCon = slua_GameFrontendHUD:Ge tPlayerController()
    local ASTExtraPlayerController = import( "/Script/ShadowTrackerExtra.STExtraPlayerController ")
    if not (isValid(uCon) and Game:IsClassOf(uCon, ASTExtraPlayerController)) then return end
    local localPlayer = GameplayData.GetPlayerCharacter(); if not isValid(localPlay er) then return end
...
print("[MOD] ✅ GOKU SERVER-SIDED PAYLOAD LOADED SUCCESSFULLY!")
